<template>
  <el-card class="box-card card">
    <div slot="header" class="clearfix">
      <el-input placeholder="例如:软件工程,2016,4,C语言实训" v-model="id">
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      let temp = this.tablearry;
      this.tablearry = temp.filter((item, index, arr) => {
    ...
    upmark() {
      ...
      this.$axios({
        url: "http://121.199.25.244:3001/worktest/mark",
        method: "post",
        data: data,
        headers: {
    ...
    searchwork() {
      ...
      this.$axios({
        url: "http://121.199.25.244:3001/worktest/scgc",
        method: "post",
        data: data,
        headers: {
          "X-Requested-With": "XMLHttpRequest",
